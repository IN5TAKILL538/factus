...
const downloadInvoicePdf = async (invoice) => {
  ...
  try {
    const response = await apiClienteFactus.get(`/v1/bills/download-pdf/${invoice.number}`);
    console.log('data', response.data);
    const pdfBase64 = response.data.data.pdf_base_64_encoded;
    console.log("Respuesta real:", response.data);
console.log("Contenido de data:", response.data?.data);
console.log("Base64 PDF:", response.data?.data?.pdf_base_64_encoded);
